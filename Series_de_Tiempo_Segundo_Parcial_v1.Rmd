...
```{r}
plot(Accidentes_transito_MEX_ts, col = "red", main = "Data histoirca anual de accidentes de transito en México de 2005 a 2021", ylab="Número de accidentes")
...
Se observa que la gran mayoria de los valores de la variable y están en el intervalo de [350,000,400,000] y que la variable puede seguir una distribución con "skewness" hacia la cola derecha  
...
La función ACF de la serie, nos refleja la forma 'original' de la serie donde los primeros años hay una tendencia  creciente y posteriormente una decreciente. El hecho de que la serie muestra varios valores significativos en el pasado sugiere que No es una serie estacionaria, para estar 100% seguros heremos un test de Dickey-Fuller  
...
#### Calculo de predicción del ARIMA para serie 1 (Accidentes transitos Mex)
...
El MAPE del modelo ARIMA 1,1,1 es de ~5.3% 
...
El MAPE del modelo SARIMA 1,1,1 (1,0,0) S=1 es de ~4.5%. Lo que indica y en este caso comprueba que diferenciar la serie hace, es decir, hacerla estacionaria, mejor en general los pronósticos en los modelos SARIMA
...
Graficamos la serie del tiempo
```{r}
plot(PEA_mex_ts, col = "blue", main = "Data histoirca anual de PEA en México de 2005 a 2022", ylab="Número de accidentes")
...
Se observa claramente una tendencia creciente (excepto en años 2021 y 2021 derivado de pandemia de Covid-19). Se observa una estacionalidad por 4trimestres donde el 1er cuatrimestre de cada año es el más bajo y e último el más alto. 
...
Se observa que la gran mayoria de los valores de la variable y están en el intervalo de [51 - 56 millones] y que la variable puede seguir una distribución con "skewness" hacia la cola izquierda  
...
#### Determinación de ACF & PACF para el caso de la serie 2 sin diferneciar 
...
El MAPE del modelo ARIMA 2,1,1 es de ~2.71% 
