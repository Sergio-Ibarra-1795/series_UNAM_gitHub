...
Tenemos data mensual de Enero de 2005 a Agosto de 2022 sobre la demanda de gas natural para el sector eléctrico en México, son 213 datos. 
La idea es usar 201 de esos 2013 para "aplicar los modelos" A) De suavizamiento "estático" y B) De "suavizamiento dinámico" o Holt-Winters para posteriormente predecir con cada modelo los valores de los 12 meses restantes y poder comparar contra los reales para detectar el mejor modelo entre los dos mencionados para modelar la demanda de gas natural en sector eléctrico. 
...
Comenzaremos tratando la serie con el método estático aplicando el siguiente flujo lógico de trabajo: 
...
Para graficar la "serie original" de demanda_electrico vs el filtro1 (Linea de tendencia) primero debemos "crear/simular" datos de Enero 2005 a Agosto 2021 a partir de nuestra linea de previamente computada de nombre: Electrico_Demanded_gas_line
...
head(Demanda_electrico_ruido)


plot(Demanda_electrico$Date, Demanda_electrico_ruido, type="l", 
...
Una vez que tenemos nuestra serie "sin los efectos de tendencia y estacionalidad" que se conoce como ruido blanco, es posible "extraer el ruido" de la serie y conocer así el error estimado (varianza)  y la desviación estandar. 
La idea es que extrayendo de la serie del ruido blanco su media histórica, estaríamos "extrayendo el ruido" y determinando la varianza, de la siguiente manera:

Varianza (errores al cuadrado) = Serie original - estacionalidad - tendecia - media. En nuestro caso, dichos errores será nuestra serie sin tendencia ni estacionalidad - media historica 

Determinemos entonces la varianza para nuestra serie de demanda_electrico 
```{r}

demanda_electrico_errores <-Demanda_electrico_ruido-mean(Demanda_electrico_ruido)
...
## Se calcula la media del ruido 
mean(Demanda_electrico_ruido)
...
Se tiene una desviación estandar de aproximadamente un 10% de la media lo cual es totalmente concordante con le hehco de que "el ruido resultante" o errores tiene una distribución normal con media cero y una desviación estandar de 0.1 
...
Para ello importamos el Data Frame con la data historica completa, es decir, la que contiene los 201 meses propuestos aquí como "train data" + los 12 meses que se pronósticaron
...
plot(Demanda_electrico_completa$Date,Demanda_electrico_pronostico, type="l", main = "Promostico de demanda de gas natrual para los siguientes 12 meses sobre el ruido blanco de la serie", , cex.main=0.8)
...
### A.5 "Devolución de efectos de tendenci y estacionalidad a los pronósticos e intervalos calculados"
...
plot(Demanda_electrico_completa$Date, Demanda_electrico_pronostico_cest, type="l", main="Demanda electrico pronostico con componente de estacionalidad + intervalos", cex.main= 0.85)
...
plot(Demanda_electrico_pronostico_cest, 
     type="l", main="Demanda electrico pronostico con componente de estacionalidad + intervalos", 
     cex.main=0.85, ylim=c(0.4,1.8))
lines(LI_demanda_electrico_cest, col="violet")
lines(LS_demanda_electrico_cest, col="violet")
...
Demanda_electrico_pronostico_ajustado<-Demanda_electrico_cest*Electrico_Demanded_gas_line_aplicada2
...
plot(Demanda_electrico_pronostico_ajustado, type="l", ylim=c(0, max(LS_demanda_electrico_ajustado, na.rm = TRUE)),  main="Demanda electrico pronostico con componente de estacionalidad y de trend + intervalos", 
     cex.main=0.85)
lines(LI_demanda_electrico_ajustado, col="violet")
lines(LS_demanda_electrico_ajustado, col="violet")
...
Se observa que los pronósticos puntuales vs los reales parecen no estar muy alejados unos de otros, sin embargo los intervalos de confianza si "sufren bastante a medida que nos alejamos del último valor real" pues son cada vez más grandes hasta llegar a abarcar un rango de [1000,6000] para el 95% de intervalo de confianza en el 12vo mes pronósticado. 
En general, con lo observado en este ejemplo y lo visto en clase se puede mencionar que: "Los métodos de suavizamiento estático son relativamente buenos para generar pronóstcos puntuales, pero los intervalos de confianza del pronóstico se ven severamente castigados mientras datos n 'en el t+n' quieran ser pronósticados"
...
cat("El MAPE error para el forecast calculado con el método de 'suavizamiento estático'de la serie de demanda de gas natural en el sector eléctrico es:", MAPE_electrico_estatico, "%")
...
### Aplicación del método de Holt Winters" a la serie de demanda de gas en el sector eléctrico 
...
### Pronóstic de la serie de demanda de gas en el sector eléctrico con el método de Holt Winters 
...
Lo que indica que el método de suavizamiento estático resultó mejor en nuestro caso para el PRONOSTICO PUNTUAL de la demanda de gas natural en el sector eléctrico en México. 
...
Se observa claramente como el método de HoltWinters otorga mejores intervalos de confianza para pronósticos incluso para aquellos 'más alejados' del último valor real. 
