---
title: "Series_de_Tiempo_Segundo_Parcial_v2"
author: "Sergibar"
date: "2023-06-01"
...
head(acf_Accidentes_transito_MEX_ts_sin_diferenciar)


pacf_Accidentes_transito_MEX_ts_sin_diferenciar <- Pacf(Accidentes_transito_MEX_ts, lag.max = 100, ci.type = "ma")
head(pacf_Accidentes_transito_MEX_ts_sin_diferenciar)
...
summary(acf_test_Accidentes_transito_MEX_ts_sin_diferenciar)
...
head(acf_PEA_mex_ts_sin_diferenciar)


pacf_PEA_mex_ts_sin_diferenciar <- Pacf(PEA_mex_ts, lag.max = 100, ci.type = "ma")
head(pacf_PEA_mex_ts_sin_diferenciar)
...
head(Demanda_electrico_original_train)
...
# Print the results
head(Demanda_electrico_ts_diferencia1)
...
## Estrategia de limitación del set de train serie 3 (Demanda electrico)  (Con data de entrenameitno hatsa 1:170 que es ignorar 2021 y 2021)


#### Training and testing sets (ORIGINAL) para aplicar modeo ARIMA serie 3 (Demanda electrico)  (Con data de entrenameitno hatsa 1:170 que es ignorar 2021 y 2021)
...
# Subset the data to keep rows from the beginning until train_end and all columns
Demanda_electrico_train_V2 <- Demanda_electrico_ts[1:170]
head(Demanda_electrico_train_V2)
tail(Demanda_electrico_train_V2)
...
Demanda_electrico_test_V2 <- Demanda_electrico_ts[202:213]
Demanda_electrico_test_V2



```

#### Ajustae y predicicon de Demanda_electrico con modelo SARIMA serie 3 (Demanda electrico)  (Con data de entrenameitno hatsa 1:170 que es ignorar 2021 y 2021)
...
# Fit the SARIMA model
ARIMA_1_1_1_Demanda_electrico_V2 <- astsa::sarima( Demanda_electrico_train_V2 , p=1, d=1, q=1)

# Print the model summary
summary(ARIMA_1_1_1_Demanda_electrico_V2)
...
pred_ARIMA_1_1_1_Demanda_electrico_V2 <- sarima.for(Demanda_electrico_train_V2, n.ahead=12,1,1,1)
pred_ARIMA_1_1_1_Demanda_electrico_V2
...
```{r}
pred_ARIMA_1_1_1_Demanda_electrico_V2$pred
```




#### Calculo de errores de serie 3 (Demanda electrico)  con modleo (Con data de entrenameitno hatsa 1:170 que es ignorar 2021 y 2021)

```{r}
accuracy(Demanda_electrico_original_test ,pred_ARIMA_1_1_1_Demanda_electrico_V2$pred )
```
...
# Fit the SARIMA model
SARIMA_PDQS_Demanda_electrico_V2_model <- Arima(Demanda_electrico_train_V2, order = c(1, 1, 1), seasonal = list(order = c(1, 1, 1), period = 12))
...
```{r}
accuracy(Demanda_electrico_original_test ,forecast_SARIMA_PDQS_Demanda_electrico_V2_model$mean )
```


## Aplicando el auto.arima a la serie 3  ORIGINAL de (Demanda_electrico)

```{r}
ARIMA_AUTO_Demanda_electrico_original_ts  <- auto.arima(Demanda_electrico_original_train)
ARIMA_AUTO_Demanda_electrico_original_ts
```


El Auto.Arima, arroja un modelo (0,1,0)

```{r}

pred_ARIMA_AUTO_Demanda_electrico_original_ts <- sarima.for(Demanda_electrico_original_train, n.ahead=12,0,1,0)
pred_ARIMA_AUTO_Demanda_electrico_original_ts
```
...
```{r}
accuracy(Demanda_electrico_original_test ,pred_ARIMA_AUTO_Demanda_electrico_original_ts$pred )
```


Aplicando el modelo más sencillo del auto.arima. El MAPE mejoró un poco respecto a la serie original que era de 18.6%%





## Aplicando el auto.arima a la serie 3 CON DATOS DE ENTRENAMIENTO AISLADOS de (Demanda_electrico)
...
pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_train_recortado <- sarima.for(Demanda_electrico_train_V2, n.ahead=12,0,1,0)
pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_train_recortado
```
...
```{r}
accuracy(Demanda_electrico_original_test ,pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_train_recortado$pred )
```


Aplicando el modelo más sencillo del auto.arima + Aislando los datos de entrenamiento para no considerar 2020 y 2021,entonces el MAPE mejoró con respecto a la serie (1,1,1) [1,1,1]S=12 que era de 12.6%%





### Transformando la variable dependiente (LOG(Y)) a la serie 3 (Demanda_electrico) ORIGINAL 
...
Demanda_electrico_ts_log <- log(Demanda_electrico_ts)
```


```{r}
plot(Demanda_electrico_ts_log, col = "brown", main = "Demanda electrico 'original'transformado usando la funcion log")
```



## Aplicando el auto.arima a la serie 3 Log ((Demanda_electrico))
...
pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_log <- sarima.for(Demanda_electrico_ts_log[1:201], n.ahead=12,0,1,0)
pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_log
```



#### Calculo de errores del  Aplicando el auto.arima a la serie 3 Log ((Demanda_electrico)) 

```{r}
accuracy(Demanda_electrico_original_test ,exp(pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_log$pred) )
```


Aplicando el logaritmo a la serie original el MAPE no mejora con respecto a la serie sin transformar 
...
### Transformando la variable dependiente (LOG(Y)) a la serie 3 (Demanda_electrico) CON DATA ENTRENAMIENTO AISLADA
...
pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_train_recortado <- sarima.for(Demanda_electrico_ts_log[1:170], n.ahead=12,0,1,0)
pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_train_recortado
```
...
```{r}
accuracy(Demanda_electrico_original_test ,exp(pred_ARIMA_AUTO_Demanda_electrico_original_ts_v_train_recortado$pred) )
```


Aplicando el logaritmo a la serie con datos de entrenamiento aislados también se consigue un "buen MAPE" con respecto a este ejercicio que es 12.4 % 
