...
Se tiene una desviación estandar de 44,547 y una  media muy cercana a cero para los errores lo cual es totalmente concordante con le hecho de que "el ruido blanco" o componente estocástico resultante tiende a tener una distribuión normal y que si a ese ruido blanco se le extrae su media tambien tiende a tener una distribución normal con media cero y en este caso una desviación estandar de 44,547 que representa aproximadamente el 10% de la variable y: Número de accidentes terrestres anuales en México 
...
Accidentes_pronostico_puntual <- rep(Accidentes_transito_ruido_blanco, times=22)
head(Accidentes_pronostico_puntual)
tail(Accidentes_pronostico_puntual)

plot(Date_accidentes2,Accidentes_pronostico_puntual, type="l", main = "Pronóstico de accidentes terrestres en MEX para los siguientes 3 años", cex.main=0.8)
```
...
plot(Date_accidentes2, accidentes_para_pron, type="l" ,main="Estimación puntual e intervalos de confianza para Accidentes terrestres MEX con el método de 'suavizamiento estático'",col="Blue", cex.main=0.7)
...
Como en este caso no tenemos componente de seasonalidad ni ne tndencia se indica que el parámetro beta y gamma tendrán un valor "false"
...
```
Lo que indica que nuestra data histórica es prácticamente una caminata aleatoria
...
```{r}
plot(accidentes_MEX_ts_h1_fc, main="Forecast de Accidentes MX usando el Método de Holt Winters con parámetro gamma ='False'", type="l", cex.main=0.8)
```
