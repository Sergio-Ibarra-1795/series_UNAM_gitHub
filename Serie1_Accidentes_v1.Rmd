...
La idea es usar 19 de esos 22 para "aplicar los modelos" A) De suavizamiento "estático" y B) De "suavizamiento dinámico" o Holt-Winters para posteriormente predecir con cada modelo los valores de los 3 años restantes y poder comparar contra los reales para detectar el mejor modelo entre los dos mencionados para modelar la los accidentes terrestres en México 
...
Accidentes_transito_MEX <-read_excel("Accidentes_transito_pp.xlsx")
```
...
```{r}
Error_est_PEA<-PEA_MX_ruido_blanco-mean(PEA_MX_ruido_blanco)
head(Error_est_PEA)
tail(Error_est_PEA)
...
Una vez teniendo nuestra seríe SIN TENDENCIA lo que implicária que "unicamente tenemos el componente estocástico o de ruido blanco en la serie" podemos proceder a llevar a cabo el pronóstico de 6 trimestres en el futuro.
Como ha sido estudiado en la clase, nuestro mejor pronóstico dadas estas condiciones en la serie es la media del ruido blanco


Vamos a hacer el pronóstico de la serie de PEA MEX para los próximos 6 trimestres
...
plot(Date_PAE2,PEA_pronostico_puntual, type="l", main = "Pronóstico de demanda de PEA en MEX para los siguientes 6 trimestres", cex.main=0.8, ylim=c(0.9, 1.1))
```
...
plot(extended_dates, PEA_original_para_pron, main="PEA pronóstico (con método estático) ajustado con componente de estacionalidad",type="l", xlab="Year", ylab="PEA", cex.main=0.8)
lines(extended_dates, Est_punt_PEA_ajustada, col="blue")


legend("bottomright", legend = c("PEA historico", "Pronóstico calculado con método estático"), 
...
# plot the original data and the predicted values together
plot(extended_dates,PEA_MEX_completa$PEA, main="PEA pronostico (con método estático) ajustada con componente de trend + intervalos vs PEA real", type="l", xlab="Year", ylab="PEA", ylim=c(40000000, 65000000), cex.main=0.7)
...
legend("bottomright", legend = c("PEA original", "Pronóstico calculado con método estático"), 
       col = c("black", "blue"), lty = 1)
