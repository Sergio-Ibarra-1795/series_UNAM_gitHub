...
## Se calcula la media del ruido blanco o componente estocástico de la serie 
...
plot(Demanda_electrico_completa$Date,Demanda_electrico_pronostico, type="l", main = "Pronóstico de demanda de gas natrual para los siguientes 12", cex.main=0.8)

```



Sobre el pronóstico puntual sin tendencia ni estacionalidad, que para el caso de la serie de demanda de gas natural en el sector eléctrico tiene un valor de:
```{r}
mean(Demanda_electrico_ruido_blanco)
```
Lo cual es bastante concordante con el hecho que. Al haber "quitado los componentes de tendencia y estacionaldad de la serie original", es decir obtenido el ruido blanco de la serie los valores de nuestra variable y: gas demandado están en este momento del algoritmo en escala de la unidad: 1. Y por lo tanto la media de dichos valores será muy cercana a 1. 
...
plot(Demanda_electrico_completa$Date,Demanda_electrico_pronostico, type="l", main = "Pronostico puntual e intervalos de confianza sin tendencia para Demanda gas natural en sector eléctrico MEX con el método de 'suavizamiento estático' ", cex.main=0.65, ylim=c(0.2,1.7))
...
plot(Demanda_electrico_pronostico_cest, 
     type="l", main="Demanda electrico pronostico (con método estático) ajustada con componente de estacionalidad + intervalos", 
...
plot(Demanda_electrico_completa$Date, Demanda_electrico_pronostico_ajustado, type="l", ylim=c(0, max(LS_demanda_electrico_ajustado, na.rm = TRUE)),  main="Demanda electrico pronostico (con método estático) ajustada con componentes de estacionalidad y de trend + intervalos vs Demanda real", 
     cex.main=0.6, col="blue")
...
legend("bottomright", legend = c("Demanda electrico original", "Pron´sstico calculado con método estático"), 
