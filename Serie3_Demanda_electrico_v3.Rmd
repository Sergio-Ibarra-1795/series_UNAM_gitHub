...
Graficamos la "data original" de demanda de gas natural en el sector eléctrico del año 2005 al 2021  
...
Para eso primero debemos "crear/simular" datos de 2005 a 2021 a partir de nuestra linea de tendencia Electrico_Demanded_gas_line
```{r}
...
#### Interpretación de los Factores estacionales FE 

Recordar que en general los factores estacionales nos dan una idea del PORCENTAJE DE AUMENTO o disminución en promedio, de la variable estudiada a lo largo de los periodos/ seasonalidades establecidas.
...
Indica que en promedio se tiene prácticamente la misma cantidad de demanda de gas natural en el sector eléctrico en estos periodos. 

Sin embargo un valor de factor estacional 3 (que representa los  meses Abril, Mayo, Junio) indica que hay un aumento aproximado del 9% en la cantidad de gas natural demanda en el sector eléctrico durante los meses de Abril, Mayo, Junio, respecto a lo observado en los de Enero, Febrero y Marzo 
...
### Revisemos los errores en el pronóstico 
...
Se añade la colum a de Demanda_electrico_completa$Demanded_Gas al Data.Frame para vidualizar los valores reales vs los estimados con la técnica aplicada 
```{r}
# Create a data frame with the predicted values and intervals
...
Calculemos el error entre los valores pronosticados con el "método de suavizamiento estático" y los valores reales de la serie 
...
Demanda_electrico_E_forecast12 <- Demanda_electrico_pronostico_fin[202:213]
...
#### Interpretación de los valores de los coeficientes del método de Holt Winters  
...
Valores de $\beta$ cercano a 0 reflejan POCOS cambios denivel a lo largo de la serie  
...
```
Lo que indica pocos cambios de nivel a lo largo de la serie o una especie de "pendiente constante"
...
Afortunadamente no obtuvimos simultaneamente valores de$\alpha$ cercanos a 1 y de $\beta$ cercanos a 1 pues eso reflejaría una CAMINATA ALEATORIA con tendencia estocástica. 
...
$$0<\gamma<1$$
Recordemos que: 
Valores de $\gamma$ cercano a 1 reflejan cambios CONSTANTES de nivel a lo largo de la serie 
Valores de $\gamma$ cercano a 0 reflejan POCOS cambios denivel a lo largo de la serie  
...
Calculemos el error entre los valores pronosticados con el "método de Holt Winters" y los valores reales de la serie 
...
#### Comparación del MAPE del pronostico hecho con métodos estáticos vs el pronóstico hecho con el método " de suavizamiento dinámico" de HoltWinters


En este caso y dado un cálculo con el métodO de HOLTWinters sin valores de arranque especificados
los errores de ambos métodos fueron los sigientes: 
...
Lo que indica que el método de suavizamiento estático resultó mejor en nuestro caso para el forecast de la demanda de gas natural en el sector eléctrico en México. 
