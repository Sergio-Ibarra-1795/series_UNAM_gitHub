---
title: "CLASS OF 07. MARCH.2023 HOLT_WINTERS (SES) vs NUESTRO FILTRO"
author: "SIR"
date: "2023-02-21"
...
#Holt
 ...
 ts_baca <- ts(Baca$Pesca, start=c(2021,1), frequency=12)
plot(ts_baca) 
...
Usando la libreria forecast para pronosticar 
```{r}
#H es el horizonte en tiempo para pronóstico
fc_hw_baca <-forecast(hw_baca, h=12, level=.90)
plot(fc_hw_baca)
...
```{r}
s_baca <- ((hw_baca$SSE)/(length(ts_baca)-1))^0.5
s_baca
```


¿CUAL ERA LA S de nuestro modelo anterior? 
...
¿Qué pasa si cambio el nivel inicial?



```{r}
hw_baca_ls_alterado1 <- HoltWinters(ts_baca, beta=FALSE, gamma=FALSE, l.start=0) 
plot(hw_baca_ls_alterado1)
hw_baca_ls_alterado1$alpha

hw_baca_ls_alterado1$SSE
```
...
```{r}
hw_baca_ls_alterado2 <- HoltWinters(ts_baca, beta=FALSE, gamma=FALSE, l.start=mean(ts_baca))
plot(hw_baca_ls_alterado2)
hw_baca_ls_alterado2$alpha
hw_baca_ls_alterado2$SSE
plot(forecast(hw_baca_ls_alterado2, h=12, level=0.95))
```

```{r}
plot(forecast(hw_baca_ls_alterado2, h=12, level=0.95), ylim=c(0,500))
```
